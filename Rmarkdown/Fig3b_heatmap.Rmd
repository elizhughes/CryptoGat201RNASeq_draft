...
counts_LFC_wide <- counts_LFC %>%
    pivot_wider(id_cols = Geneid, names_from = SampleID, values_from = LFC)

```

```{r}
counts_LFC_wide[!apply(counts_LFC_wide==0, 1, all),]
```
...
```{r}
 ...
 counts_LFC_wide[counts_LFC_wide==0] <- NA
 counts_LFC_wide
is.na(counts_LFC_wide)%>% table()
```
```{r}
counts_LFC_wide_na <- na.omit(counts_LFC_wide)
is.na(counts_LFC_wide_na)%>%table()
```


# Plot a clustered heatmap of fold changes

```{r Plot a clustered heatmap of fold changes}

Heatmap <- counts_LFC_wide_na %>%
  #filter_if(counts_LFC_wide_na, is.numeric, all_vars((.) != 0))%>%
    .[,-1] %>%
    log10() %>%
    pheatmap(main = "Heatmap of LFC",
             show_rownames = FALSE,
